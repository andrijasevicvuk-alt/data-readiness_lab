...
def get_wave2_source_config() -> Wave2SourceConfig:
    ...
    return Wave2SourceConfig(
        source_id="boatshop24",
        source_name="Boatshop24",
        base_url=base_url,
        homepage_url=f"{base_url}/en",
        listing_url=f"{base_url}/en/boats/",
        country_url_template=f"{base_url}/en/boats/country-{{slug}}/",
        target_source_note="Using obvious English-language Boatshop24 entry points and matching country-slug sale paths.",
        preferred_anchor_texts=["view details", "details", "offer"],
        detail_url_must_contain=["/boats/", "/offer/"],
        detail_url_deny_contain=["/search", "/services", "/brokers"],
        ...
        accessible_market_role="broader_marketplace_backbone",
    )
...
def run_probe(client: PoliteHttpClient):
    ...
    return probe_source(
        ...
        client=client,
        extra_notes=[
            "Wave 2 source added as a Europe-marketplace benchmark.",
        ],
    )
