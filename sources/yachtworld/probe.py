...
def get_wave2_source_config() -> Wave2SourceConfig:
    ...
    return Wave2SourceConfig(
        ...
        source_name="YachtWorld",
        base_url=base_url,
        homepage_url=f"{base_url}/",
        listing_url=f"{base_url}/boats-for-sale/",
        country_url_template=f"{base_url}/boats-for-sale/country-{{slug}}/",
        target_source_note="Using YachtWorld public sale pages as a benchmark/caution source, including direct country-slug result pages.",
        preferred_anchor_texts=["view details", "details", "contact seller"],
        detail_url_must_contain=["/boats-for-sale/", "/boat/"],
        detail_url_deny_contain=["/search", "/brokers", "/services"],
...
def run_probe(client: PoliteHttpClient):
    ...
    return probe_source(
        ...
        client=client,
        extra_notes=[
            "Wave 2 source added as a caution benchmark because it is important commercially but may be probe-hostile.",
        ],
    )
