...
def get_wave2_source_config() -> Wave2SourceConfig:
    ...
    return Wave2SourceConfig(
        source_id="boats_com",
        source_name="boats.com",
        base_url=base_url,
        homepage_url=f"{base_url}/",
        listing_url=f"{base_url}/boats-for-sale/",
        country_url_template=f"{base_url}/boats-for-sale/country-{{slug}}/",
        target_source_note="Using obvious public boats.com sale pages and country-slug paths that match the brand's public marketplace structure.",
        preferred_anchor_texts=["view details", "details", "view boat"],
...
def run_probe(client: PoliteHttpClient):
    ...
    return probe_source(
        ...
        client=client,
        extra_notes=[
            "Wave 2 source added as a broader-marketplace benchmark.",
        ],
    )
